...
#define NUM_FACES 4
#define IN_RADIUS 0.3
#define OUT_RADIUS 0.5
#define SCROLL_SPEED -0.9

#define COLOR_1 0.50, 0.30, 0.95
#define COLOR_2 0.95, 0.30, 0.10
...
vec4 slice(float x0, float x1, vec2 uv)
{
    ...
    //Checker board
    uv.y += (iTime *.4) * SCROLL_SPEED; //Scrolling
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //Twisting angle
    float angle = uvr.y + 2.0*(iTime *.4) + sin(uvr.y) * sin((iTime *.4)) * pi;
